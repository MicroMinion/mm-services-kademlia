...
Node.prototype.connect = function (contact, callback) {
  ...
  this._log.debug('entering overlay network', {
    seed: seed
  })

  async.waterfall([
    this._ensureTransportState.bind(this),
    this._router.isConnected.bind(this._router, contact),
    ...
    return done.call(self, null, contact)
  })
...
Node.prototype.ping = function (contact) {
  ...
  this._router._rpc.send(contact, ping)
}
/**
...
Node.prototype.put = function (key, value, callback) {
  ...
  this._log.debug('attempting to set value', {
    key: key
  })

  this._validateKeyValuePair(key, value, function (valid) {
    if (!valid) {
      node._log.warn('failed to validate key/value pair', {
        key: key
      })
...
Node.prototype.get = function (key, callback) {
  ...
  this._log.debug('attempting to get value', {
    key: key
  })

  self._router.findValue(key, function (err, value) {
    if (err) {
      self._log.warn('failed to get value from peers', {
        error: err.message
      })
      self._log.info('checking local storage for items', {
        key: key
      })
...
Node.prototype._putValidatedKeyValue = function (item, callback) {
  ...
  this._router.findNode(item.key, function (err, contacts) {
    if (err) {
      node._log.warn('failed to find nodes', {
        error: err.message
      })
    ...
    getContacts(contacts, function (err, contacts) {
      node._log.debug('found contacts for STORE operation', {
      ...
      async.each(contacts, function (contact, done) {
        ...
        node._log.debug('sending STORE message', {
          contact: contact
        })
...
Node.prototype._bindRPCMessageHandlers = function () {
  ...
  this._rpc.on('ready', function () {
    self._log.debug('node listening', {
      connectionInfo: self._self.toString()
    })
  })
}
...
Node.prototype._replicate = function () {
  ...
  stream.on('data', function (data) {
    ...
    try {
      ...
      return self._log.error('failed to parse value', {
        value: data.value
      })
    }
    ...
    if (item.publisher !== self._self.nodeID) {
      self._putValidatedKeyValue(item, function (err) {
        if (err) {
          self._log.error('failed to replicate item', {
            key: data.key
          })
        }
      })
    // if we are the publisher, then only replicate every T_REPUBLISH
      ...
      self.put(item.key, item.value, function (err) {
        if (err) {
          self._log.error('failed to republish item', {
            key: data.key
          })
        }
      })
  ...
  stream.on('error', function (err) {
    self._log.error('error while replicating', {
      error: err.message
    })
  })
...
Node.prototype._expire = function () {
  ...
  stream.on('data', function (data) {
    if (Date.now() <= data.value.timestamp + constants.T_EXPIRE) {
      self._storage.del(data.key, function (err) {
        if (err) {
          self._log.error('failed to expire item', {
            key: data.key
          })
        }
      })
  ...
  stream.on('error', function (err) {
    self._log.error('error while expiring', {
      error: err.message
    })
  })
...
Node.prototype._handlePing = function (incomingMsg) {
  ...
  this._log.info(
    'received PING, sending PONG', {
      sender: incomingMsg.params.contact.nodeID
    }
  )
...
Node.prototype._handleStore = function (incomingMsg) {
  ...
  this._log.info('received valid STORE', {
    sender: params.contact.nodeID
  })
...
Node.prototype._storeValidatedKeyValue = function (item, incomingMsg) {
  ...
  this._storage.put(item.key, JSON.stringify(item), function (err) {
    ...
    if (err) {
      node._log.warn('store failed, notifying', {
        contact: params.contact.nodeID
      })
    } else {
      node._log.debug('successful store, notifying', {
        contact: params.contact.nodeID
      })
    }
...
Node.prototype._handleFindNode = function (incomingMsg) {
  this._log.info('received FIND_NODE', {
    sender: incomingMsg.params.contact
  })
  ...
  this._router.getNearestContacts(
    ...
    function (err, near) {
      ...
      node._log.debug(
        'sending nearest contacts', {
          destination: params.contact.nodeID,
          amount: near.length
        }
      )
      node._rpc.send(contact, message)
...
Node.prototype._handleFindValue = function (incomingMsg) {
  ...
  this._log.info('received valid FIND_VALUE', {
    sender: params.contact.nodeID
  })
  this._storage.get(params.key, function (err, value) {
    if (err || !value) {
      node._log.debug(
        'value not found, sending contacts', {
          destination: params.contact.nodeID
        }
      )
      node._router.getNearestContacts(
    ...
    node._log.debug('found value, sending', {
      destination: params.contact.nodeID
    })
